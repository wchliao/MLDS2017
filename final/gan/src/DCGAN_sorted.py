...
from SortedDataset import DataSet
import utils
import os
import time
import csv
...
generate_num = 5
testing_file = "sample_testing_text.txt"
...
def train(batch_size):
    model_file = os.path.join('model', 'LSGANsorted_b' + str(batch_size), 'model.ckpt')
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=True):
        ...
        _, _, _ = sample_model.build_model()

        g_inputs, g_image = sample_model.build_generator()

    captions = []

    sample_path = "samples_sorted" + str(batch_size)
            ...
            if epoch != data.N_epoch:
                ...
                if epoch % 20 == 0:
                    ...
                    for vec in caption_vecs:
                        g_noise = np.random.uniform(-1, 1, [generate_num, noise_dim])
                        g_caption = [vec] * generate_num
                        generated_images.append(sess.run(g_image, feed_dict={
                            g_inputs['caption']: g_caption,
                            g_inputs['noise']: g_noise
                        }))
                    ...
                    for i, images in enumerate(generated_images, start=1):
                        for j, image in enumerate(images, start=1):
                            scipy.misc.imsave(os.path.join(sample_path, 'sample_{}_{}_{}.jpg'.format(i, j, epoch)), image)
